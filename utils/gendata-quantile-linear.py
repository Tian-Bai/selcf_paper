...
parser = argparse.ArgumentParser(description='Generate data for quantile linear regressor (L1) experiment.')
...
out_dir = f"..\\csv\\quantile-linear"
full_out_dir = f"..\\csv\\quantile-linear\\ntest={ntest} itr={itr} sigma={sigma} dim={dim}.csv"
...
def run(sig, setting, seed, **kwargs):
    ...
    reg = QuantileRegressor(quantile=1-q, solver='highs')                # solver='warn' take very long time?
    reg.fit(Xtrain, 1 * (Ytrain >= 0)) # or (Ytrain >= 0) ?
